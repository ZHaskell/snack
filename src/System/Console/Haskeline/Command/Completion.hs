...
askIMCompletions :: CommandMonad m =>
            Command m InsertMode (InsertMode, [Completion], String)
askIMCompletions (IMode xs ys) = do
    (rest, completions) <- lift $ runCompletion (withRev graphemesToString xs,
                                            graphemesToString ys)
    return (IMode (withRev stringToGraphemes rest) ys, completions, graphemesToString xs)
  where
    withRev :: ([a] -> [b]) -> [a] -> [b]
...
completionCmd k = k +> saveForUndo >|> \oldIM -> do
    (rest,cs,x) <- askIMCompletions oldIM
    let x' = Completion (reverse x) x False
    case cs of
        [] ->  effect (CompletionSelections ([],[])) >> effect RingBell >> return oldIM
        -- [c] -> setState $ useCompletion rest c
        cs -> presentCompletions k oldIM rest (x':cs) 


presentCompletions :: (MonadReader Prefs m, MonadReader Layout m)
        => Key -> InsertMode -> InsertMode
            -> [Completion] -> CmdM m InsertMode
presentCompletions k oldIM rest (c:cs) = do
    prefs <- ask
    case completionType prefs of
        MenuCompletion -> menuCompletion k (display c) (map (useCompletion rest) (c:cs)) oldIM 
        -- ListCompletion -> do
...
menuCompletion :: Monad m => Key -> String -> [InsertMode] -> Command m InsertMode InsertMode
menuCompletion k partial = loop
    where
        f (IMode x y) = reverse . graphemesToString $ x
        loop (c:cs) = \x -> do 
            y <- change (const c) x 
            effect $ CompletionSelections (if f c == reverse partial then [] else f c, map f cs)
            try (k +> loop (cs ++ [c])) y 
